...
function formatCurrency(value: number) {
  const formattedNumber = String(Math.round(value)).replace(/\B(?=(\d{3})+(?!\d))/g, ".");
...
export default async function CustomerPage({ params }: { params: { id: string } }) {
  ...
  const totalCobrado = stores.reduce(
    (sum, store) => sum + store.bills.reduce((billSum, bill) => billSum + bill.total, 0),
    0,
  );
  const totalPendiente = Math.max(0, Math.round(totalCobrado * 0.27));

  const storesWithStats = stores.map((store) => {
    const camisetas = store.bills.reduce((sum, bill) => sum + bill.products.length, 0);
              ...
              <div className="flex flex-wrap items-center gap-4">
                ...
                <div>
                  ...
                  <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
                    CUIT: 30-71234567-8 · Contacto: Juan Pérez · juan@modaurbana.com · +54 11 4567-8900
                  </p>
                </div>
              </div>
            </div>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-8 shadow-xl shadow-slate-200/50 dark:border-slate-800 dark:bg-slate-950 dark:shadow-none">
          ...
          <div className="grid gap-4 xl:grid-cols-2">
            {storesWithStats.map((store) => (
              <Card key={store.id} className="rounded-[2rem] border border-slate-200 bg-slate-50 shadow-none dark:border-slate-800 dark:bg-slate-900">
                <CardHeader>
                ...
                <CardContent>
                  <div className="grid gap-4 sm:grid-cols-3">
                    <div>
                      <div className="text-sm uppercase tracking-[0.2em] text-slate-500">Camisetas</div>
                      <div className="mt-3 text-2xl font-semibold text-slate-950 dark:text-slate-50">{store.camisetas}</div>
                    </div>
                    <div>
                      <div className="text-sm uppercase tracking-[0.2em] text-slate-500">Cobrado</div>
                      <div className="mt-3 text-2xl font-semibold text-emerald-800 dark:text-emerald-200">{formatCurrency(store.cobrado)}</div>
                    </div>
                    <div>
                      <div className="text-sm uppercase tracking-[0.2em] text-slate-500">Pendiente</div>
                      <div className="mt-3 text-2xl font-semibold text-amber-800 dark:text-amber-200">{formatCurrency(store.pendiente)}</div>
                    </div>
                  </div>
                </CardContent>
